...
export class TaskServices implements ITaskServices {
    ...
    public getEntityById = async (taskId: number): Promise<Task | null> => {
        const taskById: Task | null =
            await this.taskRepository.getEntityById(taskId)
        return taskById
    }
    public showListEntity = async (): Promise<Task[] | null> => {
        const taskList: Task[] | null =
            await this.taskRepository.showListEntity()
        return taskList
    }
    public getByUserId = async (userId: number): Promise<Task | null> => {
        const taskById: Task | null =
            await this.taskRepository.getByUserId(userId)
        return taskById
    }
}
